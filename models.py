...
class User(UserMixin, db.Model):
    ...
    is_demo = db.Column(db.Boolean, default=False)  # Демо-аккаунт
    session_token = db.Column(db.String(128), nullable=True, default=None)
    last_activity = db.Column(db.DateTime, nullable=True)  # Последняя активность
...
class Feed(db.Model):
    __tablename__ = 'feeds'
    __table_args__ = (
        db.UniqueConstraint('user_id', 'name', name='unique_feed_per_user'),
    )
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id', ondelete='CASCADE'), nullable=False)
    name = db.Column(db.String(100), nullable=False)
