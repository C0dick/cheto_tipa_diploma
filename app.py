...
from auth import auth_bp, reset_demo_data
# =============================================================================
...
def check_session_valid():
    if current_user.is_authenticated:
        ...
        if user:
            ...
            # Проверка таймаута демо-аккаунта
            if user.is_demo:
                login_time = session.get('_login_time')
                if login_time:
                    login_timestamp = datetime.strptime(login_time, '%Y-%m-%dT%H:%M:%S')
                    elapsed = (now - login_timestamp).total_seconds()
                    if elapsed > 1800:
                        ...
                        logout_user()
                        session.clear()
                        flash('⏰ Время демо-сессии истекло (30 минут). Данные очищены.', 'info')
                        return redirect(url_for('auth.login'))
        
        # Проверка токена
        if request.endpoint not in ['auth.login', 'auth.logout', 'auth.register', 'static']:
            if user and user.session_token:
                session_token = session.get('_session_token')
                if session_token != user.session_token:
                    logout_user()
                    flash('⚠️ Ваша сессия была прервана.', 'warning')
                    return redirect(url_for('auth.login'))

@app.route('/check-demo-status')
def check_demo_status():
    """Проверяет, свободен ли демо-аккаунт"""
    demo_user = User.query.filter_by(is_demo=True, username='demo').first()
    
    if not demo_user:
        return jsonify({'exists': False, 'free': False, 'message': 'Демо-аккаунт не создан'})
    
    now = datetime.utcnow()
    is_active = False
    
    # Проверяем, активен ли кто-то сейчас
    # Если нет session_token — точно свободен
    # Если есть session_token, но last_activity давно не обновлялась — тоже свободен
    if demo_user.session_token and demo_user.last_activity:
        time_diff = (now - demo_user.last_activity).total_seconds()
        is_active = time_diff < 120  # Уменьшено до 2 минут
    
    has_session = demo_user.session_token is not None
    
    # Автоматически сбрасываем токен, если время истекло
    if has_session and not is_active:
        demo_user.session_token = None
        demo_user.last_activity = None
        db.session.commit()
        has_session = False
    
    return jsonify({
        'exists': True,
        'free': not is_active and not has_session,
        'is_active': is_active,
        'has_session': has_session,
        'message': 'Демо-аккаунт свободен' if not is_active else 'Демо-аккаунт занят'
    })
...
@login_required
def feed():
    feeds = Feed.query.filter_by(user_id=current_user.id).order_by(Feed.name).all()
    ...
    recent_transactions = FeedTransaction.query.filter_by(user_id=current_user.id).order_by(FeedTransaction.date.desc()).limit(10).all()
    
    # Данные для графика по каждому корму (последние 7 дней)
    today = date.today()
    feeds_chart_data = {}
    ...
    for feed in feeds:
        ...
        for i in range(6, -1, -1):
            ...
            day_transactions = FeedTransaction.query.filter_by(
                feed_id=feed.id
            ).filter(
                db.func.date(FeedTransaction.date) == day
            ).all()
            
            total_in = sum(t.quantity for t in day_transactions if t.type == 'in')
            total_out = sum(t.quantity for t in day_transactions if t.type == 'out')
...
def add_feed():
    ...
    existing = Feed.query.filter_by(user_id=current_user.id, name=name).first()
    if existing:
        flash(f'❌ Корм с названием "{name}" уже существует!', 'danger')
        return redirect(url_for('feed'))
    ...
    # Устанавливаем базовую цену за кг
    if unit in feed.UNIT_TO_KG and feed.UNIT_TO_KG[unit] > 0 and price > 0:
        feed.base_price_per_kg = price / feed.UNIT_TO_KG[unit]
    
    db.session.add(feed)
    db.session.commit()
    
    # Если есть начальный остаток, создаем транзакцию
    if initial_stock > 0:
    ...
    flash(f'✅ Корм "{name}" добавлен! Начальный остаток: {initial_stock} {unit}', 'success')
    return redirect(url_for('feed'))
...
def edit_feed(id):
    ...
    # Проверка на уникальность названия (исключая текущий корм)
    if new_name != feed.name:
        existing = Feed.query.filter(
            Feed.user_id == current_user.id,
            Feed.name == new_name,
            Feed.id != id
        ).first()
        if existing:
            flash(f'❌ Корм с названием "{new_name}" уже существует!', 'danger')
            return redirect(url_for('feed'))
    ...
    if old_unit != new_unit:
        kg_amount = feed.convert_to_kg(feed.current_stock, old_unit)
        kg_threshold = feed.convert_to_kg(feed.min_threshold, old_unit)
        
        if old_unit in feed.UNIT_TO_KG:
            feed.base_price_per_kg = feed.price_per_unit / feed.UNIT_TO_KG[old_unit]
        
        feed.unit = new_unit
        feed.current_stock = feed.convert_from_kg(kg_amount, new_unit)
        feed.min_threshold = feed.convert_from_kg(kg_threshold, new_unit)
        feed.update_price_for_unit(new_unit)
    else:
        feed.unit = new_unit
        feed.min_threshold = float(request.form.get('min_threshold', 0))
        new_price = float(request.form.get('price', 0))
        feed.price_per_unit = new_price
        if new_unit in feed.UNIT_TO_KG:
            feed.base_price_per_kg = new_price / feed.UNIT_TO_KG[new_unit]
    
    db.session.commit()
    flash(f'✅ Корм "{feed.name}" обновлён!', 'success')
    return redirect(url_for('feed'))
...
def change_password():
    ...
    if len(new_password) < 10:
        errors.append('❌ Новый пароль должен быть не менее 10 символов')
...
@login_required
def delete_account():
    # Блокировка для демо-аккаунта
    if current_user.is_demo:
        flash('❌ Демо-аккаунт нельзя удалить!', 'danger')
        return redirect(url_for('profile'))
    ...
    # Проверка: если пользователь — админ, проверяем, есть ли другие админы
    if current_user.is_admin:
        other_admins = User.query.filter(User.is_admin == True, User.id != current_user.id).count()
        if other_admins == 0:
            flash('❌ Вы последний администратор! Назначьте другого администратора перед удалением своего аккаунта.', 'danger')
            return redirect(url_for('profile'))
    ...
    flash(f'👋 Аккаунт "{username}" успешно удалён. Будем рады видеть вас снова!', 'info')
    return redirect(url_for('auth.login'))
...
def toggle_admin(id):
    ...
    # Если снимаем админа — проверяем, что есть другие
    if user.is_admin:
        # Пользователь сейчас админ, его снимают
        other_admins = User.query.filter(User.is_admin == True, User.id != user.id).count()
        if other_admins == 0:
            flash('❌ Нельзя снять последнего администратора! Назначьте другого.', 'danger')
            return redirect(url_for('admin_panel'))
    
    user.is_admin = not user.is_admin
    db.session.commit()
    
    status = 'назначен администратором' if user.is_admin else 'снят с роли администратора'
...
def admin_delete_user(id):
    ...
    # Проверка: если удаляемый пользователь — админ, проверяем, есть ли другие админы
    if user.is_admin:
        other_admins = User.query.filter(User.is_admin == True, User.id != user.id).count()
        if other_admins == 0:
            flash('❌ Нельзя удалить последнего администратора! Назначьте другого администратора сначала.', 'danger')
            return redirect(url_for('admin_panel'))
    
    username = user.username
    
    # Удаляем пользователя (каскадно удалятся все его данные)
    db.session.delete(user)
    db.session.commit()
    
    flash(f'🗑️ Пользователь "{username}" и все его данные удалены!', 'warning')
    return redirect(url_for('admin_panel'))
